...
import { db, collection, query, where, orderBy, doc, setDoc, Timestamp, onSnapshot } from '../lib/firebase';
...
export function Resumes() {
  ...
  const createAudioFromBase64 = (base64Data: string, playbackSampleRate = 24000) => {
    ...
    try {
      ...
      d.setUint16(22, numChannels, true); // numchannels
      d.setUint32(24, playbackSampleRate, true); // samplerate
      d.setUint32(28, playbackSampleRate * numChannels * bitsPerSample / 8, true); // byterate
      d.setUint16(32, numChannels * bitsPerSample / 8, true); // blockalign
      d.setUint16(34, bitsPerSample, true); // bitspersample
      d.setUint32(36, 0x64617461, false); // "data"
      d.setUint32(40, bytes.length, true); // data size
  ...
  const generateNarrative = async () => {
    ...
    try {
      ...
      const prompt = `
        Summarize the ${deeds.length} deeds performed by ${profile?.name} during the week of ${selectedWeek}.
      ...
      const docRef = doc(db, 'users', user.uid, 'weekly_resumes', weekId);
      await setDoc(docRef, {
        text,
        weekId,
    ...
    try {
      ...
      if (base64Audio) {
        ...
        try {
          const weekId = getWeekId(new Date(selectedWeek));
          const docRef = doc(db, 'users', user.uid, 'weekly_resumes', weekId);
          await setDoc(docRef, { audioData: b64ToStore, audioCachedAt: Timestamp.now(), sampleRate: 12000 }, { merge: true });
        } catch (e) {
          console.error("Critical: Failed to save audio to Firestore:", e);
  ...
  useEffect(() => {
    ...
    const docRef = doc(db, 'users', user.uid, 'weekly_resumes', weekId);
    setIsLoading(true);
    const unsubResume = onSnapshot(docRef, (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setNarrative(data.text || '');
        setHasGeneratedThisWeek(true);
        if (data.audioData) {
          createAudioFromBase64(data.audioData, data.sampleRate || 24000);
        } else {
          setAudioUrl(null);
      ...
      console.error("Error fetching résumé:", err);
      setIsLoading(false);
    });
    ...
    const unsubDeeds = onSnapshot(q, (snapshot) => {
  ...
  return (
    <div className="py-4 md:py-8 space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <SectionHeader 
          title={t('resumes')}
          subtitle={t('weeklyJourney')}
        />

        <Select value={selectedWeek} onValueChange={(v: string | null) => v && setSelectedWeek(v)}>
          <SelectTrigger className={cn("bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 h-11 text-xs font-black uppercase tracking-widest w-full md:w-64", UI_CONSTANTS.buttonRadius)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <section className="lg:col-span-2 space-y-6">
          ...
          {error && (
            <div className={cn("p-4 bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold uppercase tracking-widest text-center", UI_CONSTANTS.cardRadius)}>
              {error}
            </div>
          )}
            ...
            <Card className={cn("bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden group", UI_CONSTANTS.cardRadius)}>
              <CardContent className="p-8 md:p-12 space-y-8">
                ...
                <div className="flex items-center gap-4 pt-8 border-t border-zinc-100 dark:border-zinc-800">
                  <Button variant="ghost" size="sm" onClick={copyToClipboard} className={cn(
                  ...
                  <Button variant="ghost" size="sm" className={cn("text-zinc-500 hover:text-zinc-900 dark:hover:text-white font-bold text-[10px] uppercase tracking-widest", UI_CONSTANTS.buttonRadius)}>
        ...
        <section className="space-y-6">
          ...
          {isLoading ? (
            ...
            <Card className={cn("bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 shadow-xl overflow-hidden relative group transition-all hover:border-indigo-500/30", UI_CONSTANTS.cardRadius)}>
            ...
            <CardContent className="p-8">
              {!narrative ? (
                ...
                <Button 
                  ...
                  className={cn("w-full bg-indigo-600 hover:bg-indigo-500 text-white h-14 text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-indigo-500/20 transition-transform active:scale-95", UI_CONSTANTS.buttonRadius)}
                >
                ...
                <div className={cn("flex flex-col items-center gap-6 bg-white dark:bg-zinc-800/50 p-6 border border-zinc-200 dark:border-zinc-700 shadow-inner relative overflow-hidden group", UI_CONSTANTS.cardRadius)}>
