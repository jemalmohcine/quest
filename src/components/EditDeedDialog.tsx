...
export function EditDeedDialog({ deed, onClose }: EditDeedDialogProps) {
  ...
  const feelingChoices =
    feeling && !baseChoices.includes(feeling) ? [feeling, ...baseChoices] : baseChoices;

  const handleSave = async () => {
    if (!user || !deed?.id || !actionName.trim()) return;
    setSaving(true);
    try {
      ...
      await updateDoc(doc(db, 'users', user.uid, 'deeds', deed.id), payload);
      onClose();
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <Dialog open={!!deed} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        ...
        <div className="space-y-5 py-2">
          <div className="space-y-2">
            ...
            <Input
              ...
              className={cn('font-semibold', UI_CONSTANTS.buttonRadius)}
            />
          </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button type="button" onClick={handleSave} disabled={saving || !actionName.trim()} className={cn('bg-indigo-600 text-white', UI_CONSTANTS.buttonRadius)}>
