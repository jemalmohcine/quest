...
export function Dashboard({
  ...
  // Dépend de user + deedsGeneration ; remonter l’onglet Dashboard relance aussi l’effet (composant remount).
  useEffect(() => {
    if (!user) return;
    ...
    const refresh = async () => {
      ...
      const [dayRes, recentRes] = await Promise.all([
        questHttp.listDeeds({ date: today }),
        questHttp.listDeeds({ recent: 4 }),
      ]);
      if (dayRes.error) console.error('Error fetching today deeds:', dayRes.error);
      if (recentRes.error) console.error('Error fetching recent deeds:', recentRes.error);
      setTodayDeeds(mapDeedRows(dayRes.data));
      setRecentDeeds(mapDeedRows(recentRes.data));
      setIsLoading(false);
    };
  ...
  return (
    <div className="py-4 md:py-8 space-y-8">
      <header className="flex items-center justify-between">
        <div className="group cursor-pointer" onClick={() => onTabChange?.('dashboard')}>
          ...
          <p className="text-zinc-500 font-bold uppercase text-[10px] tracking-[0.2em]">
            {t('welcome')}, <span className="text-zinc-900 dark:text-white">{profile?.name || 'Guest'}</span>
          </p>
        </div>
        <div
          className={cn(
            'w-12 h-12 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center shadow-xl cursor-pointer hover:border-indigo-500/50 transition-all active:scale-95 group overflow-hidden',
            UI_CONSTANTS.iconRadius,
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card
          className={cn(
            'lg:col-span-2 bg-gradient-to-br from-indigo-600 to-indigo-800 border-none shadow-2xl shadow-indigo-500/20 overflow-hidden relative group',
          ...
          {isLoading ? (
            <CardContent className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                {[1, 2, 3, 4, 5].map((i) => (
                  <Skeleton key={i} className="h-10 w-full rounded-2xl bg-white/10" />
                ))}
              </div>
            </CardContent>
          ) : (
      ...
      <section className="space-y-6">
        <SectionHeader title={t('recentActivity')} subtitle={`${recentDeeds.length} deeds logged recently`}>
          <Button
            variant="link"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {isLoading ? (
            ...
            <div
              className={cn(
                'col-span-full text-center py-12 bg-zinc-50/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 border-dashed',
                UI_CONSTANTS.cardRadius,
              )}
            >
            ...
            recentDeeds.map((deed) => (
              <DeedCard key={deed.id} deed={deed} onEdit={setDeedToEdit} onDelete={setDeedToDelete} showThought={false} />
            ))
          )}
        </div>
