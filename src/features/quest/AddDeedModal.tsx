...
export function AddDeedModal({ isOpen, onClose }: AddDeedModalProps) {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      const { data, error } = await questHttp.listDeeds({ limit: 100 });
      if (error) {
        console.error(error);
        return;
      }
      const deeds = mapDeedRows(data);
      const stats: Record<string, { count: number; lastDuration: string; lastFeeling: string; pillar: Pillar }> = {};
      for (const d of deeds) {
        const key = `${d.pillar}:${d.actionName}`;
        if (!stats[key]) {
          stats[key] = {
            count: 0,
            lastDuration: d.duration?.toString() || '',
            lastFeeling: d.feeling || 'happy',
            pillar: d.pillar,
          };
        }
        stats[key].count += 1;
      ...
      const payload = {
        ...
        ...(duration
          ? (() => {
              ...
              return !Number.isNaN(du) ? { duration: Math.max(0, du) } : {};
            })()
          : {}),
      };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6 pb-[max(2rem,calc(1rem+env(safe-area-inset-bottom,0px)))]">
