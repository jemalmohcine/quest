...
import { loginSchema, resetPasswordSchema, signUpSchema } from '@/lib/schemas/auth';

/** Évite les open-redirect via ?next= */
...
export function AuthForms({ onBack }: { onBack: () => void }) {
  const { signIn, signInEmail, signUpEmail, resetPassword, t } = useQuest();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        await signInEmail(parsed.data.email, parsed.data.password);
        ...
        await signUpEmail(parsed.data.email, parsed.data.password, parsed.data.name);
